...
public class MinerDriver {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            numGenerations = Integer.parseInt(args[4]);
        } catch (NumberFormatException nfe) {
            logger.info("Number of generations or population size are not correct");
            return;
        }
        if (populationSize < 600) {
            logger.info("Population size should be at least 600");
            return;
        }
        if (numGenerations < 1) {
            logger.info("Number of generations should be at least 1");
            return;
        }
        driver.setPopulationSize(populationSize);
        driver.setNumGenerations(numGenerations);
        ...
        try {
            ...
            logger.info("A mistake was made. Try once more");
        }
    ...
    private JobConf createJob(int i) {
        ...
        job.set("mapreduce.tasktracker.map.tasks.maximum", "" + NUM_REDUCERS);
        job.set("mapreduce.tasktracker.reduce.tasks.maximum", "" + NUM_REDUCERS);
